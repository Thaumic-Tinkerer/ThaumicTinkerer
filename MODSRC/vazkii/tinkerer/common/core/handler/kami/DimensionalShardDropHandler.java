...
public class DimensionalShardDropHandler {
    ...
    public void onEntityLivingDrops(LivingDropsEvent event) {
        if(event.source.getEntity() != null && event.source.getEntity() instanceof EntityPlayer) {
        	if(event.entityLiving instanceof EntityEnderman && event.entityLiving.dimension == 1 && Math.random() > 1D / 32D)
        	...
        	if(event.entityLiving instanceof EntityPigZombie && event.entityLiving.dimension == -1 && Math.random() > 1D / 16D)
