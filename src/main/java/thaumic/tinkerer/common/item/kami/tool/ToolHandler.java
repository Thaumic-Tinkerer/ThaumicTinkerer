...
public final class ToolHandler {
  ...
  public static Material[] materialsPick =
      new Material[] {Material.rock,  Material.iron,   Material.ice,
                      Material.glass, Material.piston, Material.anvil};
  public static Material[] materialsShovel =
      new Material[] {Material.grass, Material.ground,      Material.sand,
                      Material.snow,  Material.craftedSnow, Material.clay};
  public static Material[] materialsAxe = new Material[] {
      Material.coral, Material.leaves, Material.plants, Material.wood};

  public static int getMode(ItemStack tool) { return tool.getItemDamage(); }

  public static int getNextMode(int mode) { return mode == 2 ? 0 : mode + 1; }
  ...
  public static boolean isRightMaterial(Material material,
  ...
  public static void removeBlocksInIteration(EntityPlayer player, World world,
                                             int x, int y, int z, int xs,
                                             int ys, int zs, int xe, int ye,
                                             int ze, Block block,
                                             Material[] materialsListing,
                                             boolean silk, int fortune) {
    MovingObjectPosition mop =
        raytraceFromEntity(player.worldObj, player, false, 4.5d);
    if (mop == null)
      return;
    ...
    for (int x1 = xs; x1 < xe; x1++) {
      for (int y1 = ys; y1 < ye; y1++) {
        for (int z1 = zs; z1 < ze; z1++) {
          ...
          // ToolHandler.removeBlockWithDrops(player, world, x1 + x, y1 + y, z1
          // + z, x, y, z, lock2, materialsListing, silk, fortune,
          // blockHardness,metadata);
          breakExtraBlock(player.worldObj, x1 + x, y1 + y, z1 + z, player, x, y,
                          z, materialsListing);
        }
      }
    }
    List list = world.getEntitiesWithinAABB(
        EntityItem.class, AxisAlignedBB.getBoundingBox(x + xs, y + ys, z + zs,
                                                       x + xe, y + ye, z + ze));
  ...
  protected static void breakExtraBlock(World world, int x, int y, int z,
                                        EntityPlayer playerEntity, int refX,
                                        int refY, int refZ,
                                        Material[] materialsListing) {
    ...
    if (!block.canHarvestBlock(player, meta) ||
        !isRightMaterial(block.getMaterial(), materialsListing))
      return;
    Block refBlock = world.getBlock(refX, refY, refZ);
    float refStrength =
        ForgeHooks.blockStrength(refBlock, player, world, refX, refY, refZ);
    ...
    // callback to the tool the player uses. Called on both sides. This damages
    // the tool n stuff.
    player.getCurrentEquippedItem().func_150999_a(world, block, x, y, z,
                                                  player);
    ...
    if (!world.isRemote) {
      ...
      if (block.removedByPlayer(
              world, player, x, y, z,
              true)) // boolean is if block can be harvested, checked above
      {
        block.onBlockDestroyedByPlayer(world, x, y, z, meta);
    ...
    // client sided handling
      ...
      //  clientside we do a "this clock has been clicked on long enough to be
      //  broken" call. This should not send any new packets the code above,
      //  executed on the server, sends a block-updates that give us the correct
      //  state of the block we destroy.
  ...
  public static void removeBlockWithDrops(EntityPlayer player, World world,
                                          int x, int y, int z, int bx, int by,
                                          int bz, Block block,
                                          Material[] materialsListing,
                                          boolean silk, int fortune,
                                          float blockHardness, int metadata) {
  ...
  public static String getToolModeStr(IAdvancedTool tool, ItemStack stack) {
    return StatCollector.translateToLocal("ttmisc.mode." + tool.getType() +
                                          "." + ToolHandler.getMode(stack));
  }
  ...
  raytraceFromEntity(World world, Entity player, boolean par3, double range) {
    float f = 1.0F;
