...
public final class ConfigHandler {
  ...
  public static void loadConfig(File configFile) {
    ...
    Property propEnableKami =
        config.get(Configuration.CATEGORY_GENERAL, "kami.forceenabled", true);
    propEnableKami.comment =
    ...
    enableKami = Loader.isModLoaded("ThaumicTinkererKami") ||
                 propEnableKami.getBoolean(true);

    Property propEnableTooltips = config.get(Configuration.CATEGORY_GENERAL,
                                             "tooltipIndicators.enabled", true);
    propEnableTooltips.comment =
        "Set to false to disable the [TT] tooltips in the Thaumonomicon.";
    useTootlipIndicators = propEnableTooltips.getBoolean(true);

    Property propEnableSurvivalShareTome = config.get(
        Configuration.CATEGORY_GENERAL, "shareTome.survival.enabled", true);
    propEnableSurvivalShareTome.comment =
    ...
    Property propEasymodeResearch = config.get(
        Configuration.CATEGORY_GENERAL, "research.easymode.enabled", false);
    propEasymodeResearch.comment =
    ...
    Config.researchDifficulty =
        (enableEasymodeResearch) ? -1 : Config.researchDifficulty;

    Property propDebugCommands = config.get(Configuration.CATEGORY_GENERAL,
                                            "debugCommands.enabled", false);
    propDebugCommands.comment = "Set to true to enable debugging commands.";
    enableDebugCommands = propDebugCommands.getBoolean(false);

    Property propEnableFlight =
        config.get(Configuration.CATEGORY_GENERAL, "modFlight.enabled", true);
    propEnableFlight.comment = "Set to true to enable flight in this mod.";
    enableFlight = propEnableFlight.getBoolean(true);

    Property propRepairTCon = config.get(Configuration.CATEGORY_GENERAL,
                                         "repairTconTools.enabled", false);
    propRepairTCon.comment =
        "Can Thaumic Tinkerer repair Tinkers Construct tools.";
    repairTConTools = propRepairTCon.getBoolean(false);

    Property propOreDict = config.get(Configuration.CATEGORY_GENERAL,
                                      "oreDictMetal.enabled", true);
    propOreDict.comment =
        "Set to false to disable usage of ore dictionary metals (tin and copper).";
    useOreDictMetal = propOreDict.getBoolean(true);

    Property propImbuedFire =
        config.get(Configuration.CATEGORY_GENERAL, "imbuedFire.enabled", true);
    enableFire = propImbuedFire.getBoolean(true);

    Property propImbuedFireCake = config.get(Configuration.CATEGORY_GENERAL,
                                             "imbuedFire.cake.enabled", true);
    propImbuedFireCake.comment =
    ...
    Property propCropsAllowBonemeal = config.get(
        Configuration.CATEGORY_GENERAL, "cropsAllowBonemeal.enabled", false);
    propCropsAllowBonemeal.comment =
    ...
    // Potions
    Property propFirePotionId =
        config.get(CATEGORY_POTIONS, "Fire Potion id", potionFireId);
    propFirePotionId.comment = "Set to the potion id for fire potion";
    potionFireId = propFirePotionId.getInt(potionFireId);

    Property propAirPotionId =
        config.get(CATEGORY_POTIONS, "Air Potion id", potionAirId);
    propAirPotionId.comment = "Set to the potion id for air potion";
    potionAirId = propAirPotionId.getInt(potionAirId);

    Property propWaterPotionId =
        config.get(CATEGORY_POTIONS, "Water Potion id", potionWaterId);
    propWaterPotionId.comment = "Set to the potion id for water potion";
    potionWaterId = propWaterPotionId.getInt(potionWaterId);

    Property propEarthPotionId =
        config.get(CATEGORY_POTIONS, "Earth Potion id", potionEarthId);
    propEarthPotionId.comment = "Set to the potion id for earth potion";
    potionEarthId = propEarthPotionId.getInt(potionEarthId);

    if (enableKami) {
      Property propDimensionID =
          config.get(CATEGORY_KAMI_GENERAL, "Bedrock dimension id", -19);
      propDimensionID.comment =
          "Set to the dimension id wished for bedrock dimension, or 0 to disable";
      bedrockDimensionID = propDimensionID.getInt(-19);

      Property oreBlacklist =
          config.get(CATEGORY_KAMI_GENERAL, "Bedrock dimension ore Blacklist",
                     new String[] {"oreFirestone"});
      oreBlacklist.comment =
          "These oredict tags will not be used to spawn ores in the bedrock dimension";
      OreClusterGenerator.blacklist = oreBlacklist.getStringList();

      Property propOreDensity = config.get(Configuration.CATEGORY_GENERAL,
                                           "Bedrock Dimension ore density", 1);
      propOreDensity.comment =
          "The number of vertical veins of ore per chunk. Default: 1";
      OreClusterGenerator.density = propOreDensity.getInt(1);

      Property propShowPlacementMirrorBlocks = config.get(
          CATEGORY_KAMI_GENERAL, "placementMirror.blocks.show", true);
      propShowPlacementMirrorBlocks.comment =
          "Set to false to remove the phantom blocks displayed by the Worldshaper's Seeing Glass.";
      showPlacementMirrorBlocks =
          propShowPlacementMirrorBlocks.getBoolean(true);

      Property propNetherID =
          config.get(CATEGORY_KAMI_GENERAL, "dimension.nether.id", -1);
      propNetherID.comment =
      ...
      Property propEndID =
          config.get(CATEGORY_KAMI_GENERAL, "dimension.end.id", 1);
      propEndID.comment =
          "The Dimension ID for the End, leave at 1 if you don't modify it with another mod/plugin.";
      endDimensionID = propEndID.getInt(1);
    }

    LibEnchantIDs.idAscentBoost =
        loadEnchant(LibEnchantNames.ASCENT_BOOST, LibEnchantIDs.idAscentBoost);
    LibEnchantIDs.idSlowFall =
        loadEnchant(LibEnchantNames.SLOW_FALL, LibEnchantIDs.idSlowFall);
    LibEnchantIDs.idAutoSmelt =
        loadEnchant(LibEnchantNames.AUTO_SMELT, LibEnchantIDs.idAutoSmelt);
    LibEnchantIDs.idDesintegrate =
        loadEnchant(LibEnchantNames.DESINTEGRATE, LibEnchantIDs.idDesintegrate);
    LibEnchantIDs.idQuickDraw =
        loadEnchant(LibEnchantNames.QUICK_DRAW, LibEnchantIDs.idQuickDraw);
    LibEnchantIDs.idVampirism =
        loadEnchant(LibEnchantNames.VAMPIRISM, LibEnchantIDs.idVampirism);

    LibEnchantIDs.focusedStrike =
        loadEnchant(LibEnchantNames.focusedStrike, LibEnchantIDs.focusedStrike);
    LibEnchantIDs.dispersedStrikes = loadEnchant(
        LibEnchantNames.dispersedStrikes, LibEnchantIDs.dispersedStrikes);
    LibEnchantIDs.finalStrike =
        loadEnchant(LibEnchantNames.finalStrike, LibEnchantIDs.finalStrike);
    LibEnchantIDs.valiance =
        loadEnchant(LibEnchantNames.valiance, LibEnchantIDs.valiance);
    LibEnchantIDs.pounce =
        loadEnchant(LibEnchantNames.pounce, LibEnchantIDs.pounce);
    LibEnchantIDs.shockwave =
        loadEnchant(LibEnchantNames.shockwave, LibEnchantIDs.shockwave);
    LibEnchantIDs.shatter =
        loadEnchant(LibEnchantNames.shatter, LibEnchantIDs.shatter);
    LibEnchantIDs.tunnel =
        loadEnchant(LibEnchantNames.tunnel, LibEnchantIDs.tunnel);

    config.save();
  }

  private static int loadEnchant(String label, int deafultID) {
    return config.get(CATEGORY_ENCHANTMENTS, "id_enchant." + label, deafultID)
        .getInt(deafultID);
  }
}
