...
public class TileEnchanter
    extends TileEntity implements ISidedInventory, IMovableTile {
  ...
  public void appendEnchant(int enchant) { enchantments.add(enchant); }

  public void appendLevel(int level) { levels.add(level); }

  public void removeEnchant(int index) { enchantments.remove(index); }

  public void removeLevel(int index) { levels.remove(index); }

  public void setEnchant(int index, int enchant) {
    enchantments.set(index, enchant);
  }

  public void setLevel(int index, int level) { levels.set(index, level); }
  ...
  public void updateEntity() {
    ...
    enchantItem : {
      ...
      enchantments.clear();
      levels.clear();
      worldObj.playSoundEffect(xCoord, yCoord, zCoord, "thaumcraft:wand", 1F,
                               1F);
      ...
      if (wand != null && wand.getItem() instanceof ItemWandCasting &&
        ...
        for (Aspect aspect : LibFeatures.PRIMAL_ASPECTS) {
          missing =
              totalAspects.getAmount(aspect) - currentAspects.getAmount(aspect);
        ...
        int i = aspectsThatCanGet.isEmpty()
                    ? 0
                    : worldObj.rand.nextInt(aspectsThatCanGet.size());
        Aspect aspect =
            aspectsThatCanGet.isEmpty() ? null : aspectsThatCanGet.get(i);

        if (aspect != null) {
          this.consumeAllVisCrafting(
              wand, null, new AspectList().add(aspect, 1), true, wandItem);
          currentAspects.add(aspect, 1);
          Tuple4Int p = pillars.get(i);
          if (worldObj.rand.nextBoolean()) {
            Thaumcraft.proxy.blockRunes(worldObj, p.i1, p.i4 - 0.75, p.i3,
                                        0.3F + worldObj.rand.nextFloat() * 0.7F,
                                        0.0F,
                                        0.3F + worldObj.rand.nextFloat() * 0.7F,
                                        15, worldObj.rand.nextFloat() / 8F);
            Thaumcraft.proxy.blockRunes(worldObj, xCoord, yCoord + 0.25, zCoord,
                                        0.3F + worldObj.rand.nextFloat() * 0.7F,
                                        0.0F,
                                        0.3F + worldObj.rand.nextFloat() * 0.7F,
                                        15, worldObj.rand.nextFloat() / 8F);
            if (worldObj.rand.nextInt(5) == 0)
              worldObj.playSoundEffect(p.i1, p.i2, p.i3, "thaumcraft:brain",
                                       0.5F, 1F);
          }
        }
      }
  ...
  public boolean consumeAllVisCrafting(ItemStack is, EntityPlayer player,
                                       AspectList aspects, boolean doit,
    ...
    if (aspects != null && aspects.size() != 0) {
      ...
      if (aspects != null && aspects.size() != 0) {
        ...
        for (i$ = 0; i$ < len$; ++i$) {
          ...
          cost = (int)((float)cost * wandItem.getConsumptionModifier(
                                         is, player, aspect, true));
          nl.add(aspect, cost);
        }
        ...
        if (doit &&
          ...
          for (i$ = 0; i$ < len$; ++i$) {
            aspect = arr$[i$];
            wandItem.storeVis(
                is, aspect, wandItem.getVis(is, aspect) - nl.getAmount(aspect));
          }
        }
  ...
  public void updateAspectList() {
    ...
    for (int i = 0; i < enchantments.size(); i++) {
      ...
      AspectList aspects =
          EnchantmentManager.enchantmentData.get(enchant).get(level).aspects;
  ...
  public void readCustomNBT(NBTTagCompound par1NBTTagCompound) {
    ...
    currentAspects.readFromNBT(
        par1NBTTagCompound.getCompoundTag(TAG_CURRENT_ASPECTS));
    totalAspects.readFromNBT(
        par1NBTTagCompound.getCompoundTag(TAG_TOTAL_ASPECTS));
    ...
    NBTTagList var2 =
        par1NBTTagCompound.getTagList("Items", Constants.NBT.TAG_COMPOUND);
  ...
  public void writeCustomNBT(NBTTagCompound par1NBTTagCompound) {
    ...
    par1NBTTagCompound.setIntArray(TAG_ENCHANTS,
                                   ArrayUtils.toPrimitive(enchantments.toArray(
                                       new Integer[enchantments.size()])));
  ...
  public S35PacketUpdateTileEntity getDescriptionPacket() {
    ...
    return new S35PacketUpdateTileEntity(xCoord, yCoord, zCoord, -999,
                                         nbttagcompound);
  }
